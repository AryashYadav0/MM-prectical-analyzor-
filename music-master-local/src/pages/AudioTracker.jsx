...
export default function AudioTracker() {
  ...
  const timeDataRef = useRef(new Float32Array(2048));
  ...
  const drawGraph = useCallback((points, start, end) => {
    ...
    const validHz = points.filter(p => p.hz > 0).map(p => p.hz);
    const minHz = validHz.length > 0 ? Math.min(...validHz) : 50;
    const maxHz = validHz.length > 0 ? Math.max(...validHz) : 1000;
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      ctx.fillText(`${Math.round(maxHz - (hzRange * i) / 4)}Hz`, 4, y - 3);
    }
  ...
  const analyze = useCallback(() => {
    ...
    let rms = 0;
    for (let i = 0; i < bufferRef.current.length; i++) rms += bufferRef.current[i] ** 2;
    ...
    setDataPoints(prev => {
      const updated = [...prev, { time: now, hz, amplitude: rms }].slice(-500);
  ...
  const markStart = () => setStartTime(elapsedMs);
  const markEnd   = () => setEndTime(elapsedMs);
  const clearMarkers = () => { setStartTime(null); setEndTime(null); };
  const clearData = () => {
    setDataPoints([]);
    setStartTime(null);
    setEndTime(null);
  ...
  useEffect(() => { drawGraph(dataPoints, startTime, endTime); }, [startTime, endTime, drawGraph, dataPoints]);

  const filtered = dataPoints.filter(p =>
    (startTime === null || p.time >= startTime) &&
    (endTime === null   || p.time <= endTime)
  );
  const validFiltered = filtered.filter(p => p.hz > 0);
  const avgHz    = validFiltered.length ? Math.round(validFiltered.reduce((a, b) => a + b.hz, 0) / validFiltered.length * 10) / 10 : 0;
  const maxHzVal = filtered.length ? Math.max(...filtered.map(p => p.hz)) : 0;
  const minHzVal = validFiltered.length ? Math.min(...validFiltered.map(p => p.hz)) : 0;
  const duration = startTime !== null && endTime !== null ? ((endTime - startTime) / 1000).toFixed(2) : null;

  const card = { background: "#111827", borderRadius: 12, padding: "16px 12px", textAlign: "center" };
  const label = { fontSize: 11, color: "#64748b", marginBottom: 6, letterSpacing: 1 };
  const bigNum = (color) => ({ fontSize: 32, fontWeight: 800, color, lineHeight: 1 });
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
          <div style={{ ...card, border: "1px solid #1e3a5f" }}>
            ...
            <div style={bigNum("#22d3ee")}>{currentHz > 0 ? currentHz : "---"}</div>
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>Hz</div>
          </div>

          <div style={{ ...card, border: "1px solid #2d1f5e" }}>
            <div style={label}>MUSICAL NOTE</div>
            <div style={bigNum("#a78bfa")}>
          ...
          <div style={{ ...card, border: "1px solid #1f3a2d" }}>
            <div style={label}>AMPLITUDE</div>
            <div style={{ height: 40, background: "#0f172a", borderRadius: 8, overflow: "hidden", display: "flex", alignItems: "flex-end" }}>
              <div style={{
                width: `${amplitude * 100}%`, height: "100%", borderRadius: 8,
        ...
        <div style={{ background: "#111827", border: "1px solid #1e2d40", borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={label}>LIVE WAVEFORM</div>
        ...
        {/* Pitch Graph */}
        <div style={{ background: "#111827", border: "1px solid #1e2d40", borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={label}>PITCH GRAPH (Hz over time)</div>
