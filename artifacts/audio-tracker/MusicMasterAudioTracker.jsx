...
export default function MusicMasterAudioTracker() {
  ...
  const drawGraph = useCallback((points, start, end) => {
    ...
    const maxHz = points.length > 0 ? Math.max(...points.map(p => p.hz)) : 1000;
  ...
  const analyze = useCallback(() => {
    ...
    setDataPoints(prev => {
      const newPoint = { time: now, hz, amplitude: rms };
  ...
  const validFiltered = filteredPoints.filter(p => p.hz > 0);
  const avgHz = validFiltered.length > 0
    ? Math.round(validFiltered.reduce((a, b) => a + b.hz, 0) / validFiltered.length * 10) / 10
    : 0;
  const maxHzVal = filteredPoints.length > 0 ? Math.max(...filteredPoints.map(p => p.hz)) : 0;
  const minHzVal = validFiltered.length > 0 ? Math.min(...validFiltered.map(p => p.hz)) : 0;
